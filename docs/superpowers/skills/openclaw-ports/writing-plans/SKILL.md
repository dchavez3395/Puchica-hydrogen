...
context for our codebase and questionable taste. Document everything
they need to know: which files to touch for each task, code, testing,
docs they might need to check, how to test it. Give them the whole plan
...
**Save plans to:** `C:\Users\dchav\.openclaw\workspace\plans\YYYY-MM-DD-<feature-name>.md`
- (User preferences for plan location override this default. For Puchica
  repo work, the Puchica convention is `docs/superpowers/plans/...`.)
...
- You reason best about code you can hold in context at once, and your
  edits are more reliable when files are focused. Prefer smaller,
  focused files over large ones that do too much.
...
- In existing codebases, follow established patterns. If the codebase
  uses large files, don't unilaterally restructure — but if a file
  you're modifying has grown unwieldy, including a split in the plan
  is reasonable.

This structure informs the task decomposition. Each task should
...
worth a fresh reviewer's gate. When drawing task boundaries: fold
setup, configuration, scaffolding, and documentation steps into the
task whose deliverable needs them; split only where a reviewer could
meaningfully reject one task while approving its neighbor. Each task
...
> **For agentic workers:** REQUIRED SUB-SKILL: Use OpenClaw's
> subagent-driven-development or executing-plans equivalent to implement
...
**"Plan complete and saved to `<path>/<filename>.md`. Two execution options:**

**1. Subagent-Driven (recommended)** — fresh subagent per task, review
between tasks, fast iteration

**2. Inline Execution** — execute tasks in this session, batch with
checkpoints
...
Source: `C:\Users\dchav\.claude\skills\writing-plans\SKILL.md` (Anthropic).
The plan format is platform-agnostic. OpenClaw adaptations:
- Plan save path defaults to `C:\Users\dchav\.openclaw\workspace\plans\`
  (Puchica convention: `E:\Claude\puchica-site\docs\superpowers\plans\`)
- "subagent-driven-development" and "executing-plans" referenced as
  the recommended execution skills, but OpenClaw doesn't have native
  versions yet. The behavior is approximated by `sessions_spawn` for
  sub-agents and direct `update_plan` for inline.

The references folder in the source (`plan-document-reviewer-prompt.md`)
is not ported — use the Self-Review section above as the inline
checklist.
