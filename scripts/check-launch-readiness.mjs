...
export function runLaunchChecks() {
  const envPath = process.env.LAUNCH_ENV_FILE
    ? path.resolve(process.env.LAUNCH_ENV_FILE)
    : path.join(rootDir, '.env');
  const evidenceDir = process.env.LAUNCH_EVIDENCE_DIR
    ? path.resolve(process.env.LAUNCH_EVIDENCE_DIR)
